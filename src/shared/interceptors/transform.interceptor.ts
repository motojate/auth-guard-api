...
export class TransformInterceptor<T> implements NestInterceptor<T, BaseResponse<T>> {
  intercept(context: ExecutionContext, next: CallHandler): Observable<BaseResponse<T | null>> {
    return next.handle().pipe(map((data) => this.createResponse(data)));
  }

  private createResponse(data?: T): BaseResponse<T | null> {
    return this.isEmpty(data) ? BaseResponse.createEmptyResponse() : BaseResponse.createSuccessResponse(data);
  }
